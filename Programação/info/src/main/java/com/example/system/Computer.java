...
public class Computer {
    ...
    public List<List<String>> listarDispositivos(String ip) {
        ...
        try {
            String[] comandos = makeCommand("listDevices2.cmd");

            processBuilder = new ProcessBuilder(comandos);
            Process process = processBuilder.start();
            
            List<String> listResults = mostrarResultado(process);
    ...
    // public List<List<String>> listarDispositivos(String ogIp) {
        
    //     List<List<String>> infoNmap = new ArrayList<>();
    //     try {

    //         Pattern pattern = Pattern.compile("^.*(?=\\.\\d+$)");
    //         Matcher matcher = pattern.matcher(ogIp);
    //         matcher.find();
            
    //         String newIp = matcher.group(1); 

    //         String[] comandos = makeCommand("listDevices2.cmd", newIp);
    ...
    //         List<String> resultados = mostrarResultado(process).stream()
    //             .filter(result -> result.toLowerCase().contains("nmap scan") ||result.toLowerCase().contains("mac"))
    //                 .collect(Collectors.toList());


    //         List<String> ipMac = new ArrayList<>();
    //         for (String result : resultados) {
    //             if (result.contains("nmap")) ipMac.add(result.split(" ")[4]);
    //             else if (result.contains("mac")) {
    //                 ipMac.add(result.split(" ")[2]);
    //                 infoNmap.add(ipMac);
    //             }
    //         }    
                        
    //         for (List<String> info : infoNmap) {
    ...
    private List<String> getRede() {
        ...
        try {
            String[] comandos = makeCommand("rede.cmd");

            processBuilder = new ProcessBuilder(comandos);
            Process process = processBuilder.start();
            
            listResults = (mostrarResultado(process));
    ...
    private String[] makeCommand(String file, String params) {
        String[] command;
        ...
        if (!isWindows) {
            ...
            String scriptPath = directory + "\\cmd\\"+ file;
            command = formatar.commandCommandPrompt(scriptPath, params); 
        }

        return command;
    }

    private List<String> mostrarResultado(Process process) {
